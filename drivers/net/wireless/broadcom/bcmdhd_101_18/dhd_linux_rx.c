...
#undef ETHER_ICMPV6_TYPE
		...
		if (ntoh16(skb->protocol) != ETHER_TYPE_BRCM) {
			...
			unsigned long lock_rpm_flags = 0;
...
#ifdef RPM_FAST_TRIGGER
			/* Reset RPM fast trigger if rx frame */
			DHD_GENERAL_LOCK(dhdp, lock_rpm_flags);
			if (dhdp->rpm_fast_trigger) {
				DHD_ERROR(("%s: Reset RPM fast trigger\n", __FUNCTION__));
				dhdp->rpm_fast_trigger = FALSE;
			}
			dhdp->last_tx_rx = jiffies;
			DHD_GENERAL_UNLOCK(dhdp, lock_rpm_flags);
#endif /* RPM_FAST_TRIGGER */
		}
