...
static void copy_grayspot_cal_maptbl(struct maptbl *tbl, u8 *dst)
{
	...
	if (panel_data->props.grayspot) {
		dst[1] = GRAYSPOT_START_SWIRE;
		dst[2] = GRAYSPOT_CAL_OFFSET;
	}
	panel_info("grayspot_cal 0x%02x, 0x%02x, 0x%02x\n",
		dst[0], dst[1], dst[2]);
}
#endif
...
static int __init s6e3fc3_panel_init(void)
{
	register_common_panel(&s6e3fc3_r9_a4_s0_panel_info);
	return 0;
}

static void __exit s6e3fc3_panel_exit(void)
{
	deregister_common_panel(&s6e3fc3_r9_a4_s0_panel_info);
}
